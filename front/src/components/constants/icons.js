
const plus = require('../../assets/icons/plus.png');
const info = require('../../assets/icons/info.png');
const book = require('../../assets/icons/book.png');
const right = require('../../assets/icons/right.png');
const left = require('../../assets/icons/left.png');
const play = require('../../assets/icons/play.png');
const pause = require('../../assets/icons/pause.png');
const drawer = require('../../assets/icons/drawer.png');
const notif = require('../../assets/icons/notif.png');
const book2 = require('../../assets/icons/book2.png');
const book1 = require('../../assets/icons/book1.png');
const coeur = require('../../assets/icons/coeur.png');
const live = require('../../assets/icons/live.png');
const back = require('../../assets/icons/back.png');

export default {plus,info,book,right,left,play,pause,coeur,book1,book2,notif,drawer,live,back};
